...
* **Actions Completed:**
  1. Extracted alter-to-alter ties from `Data/alter_alter_ties_longitudinal.rds` ($N = 29,473$ ties) and calculated the common neighbors / shared contacts of each alter within ego's nominated network ($0$ to $19$, Mean $= 5.43$, SD $= 4.29$), as well as normalized closure ratio ($0$ to $1$, Mean $= 0.487$).
