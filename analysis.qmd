...
mod_opac_int_closed <- glmer(
  persisted ~ num_match_closed * num_unknown + open_match_count + same_dorm + is_friend + race_homophily + freq_factor + 
    female_factor * alterfemale_factor + close_factor + duration_ + period + (1 | egoid),
  data = df_period, family = binomial(link = "logit"), control = glmer_ctrl, nAGQ = 0
)
mod_opac_int_open <- glmer(
  persisted ~ num_match_closed + open_match_count * num_unknown + same_dorm + is_friend + race_homophily + freq_factor + 
    female_factor * alterfemale_factor + close_factor + duration_ + period + (1 | egoid),
  data = df_period, family = binomial(link = "logit"), control = glmer_ctrl, nAGQ = 0
)
mod_opac_int_both <- glmer(
  persisted ~ num_match_closed * num_unknown + open_match_count * num_unknown + same_dorm + is_friend + race_homophily + freq_factor + 
    female_factor * alterfemale_factor + close_factor + duration_ + period + (1 | egoid),
  data = df_period, family = binomial(link = "logit"), control = glmer_ctrl, nAGQ = 0
)
```
...
library(knitr)
desc_data <- readRDS(here::here("data", "processed", "descriptives.rds"))
kable(desc_data$cont, digits = 2)
```
```{r}
#| label: tbl-descriptives-cat
#| tbl-cap: "Descriptive Statistics for Categorical Variables"
kable(desc_data$cat, digits = 1)
```
