...
df_period <- readRDS(here("data", "processed", "adjacent_waves.rds")) %>%
  ...
  mutate(
    ...
    # We recode close=4 ("Don't know/Acquaintance") into 3 ("Not Close") to preserve valid ties
    ...
    # Center and scale continuous duration variables to speed up optimizer convergence
    duration_c = scale(duration_)[,1],
    duration_sq_c = scale(duration_^2)[,1],
    # Structural embeddedness (common neighbors in ego network, standardized with period FE absorbing unmeasured wave 6)
...
```
## Main Effects Models
The following models assess the main effects of closed-form cultural matching, open-ended cultural matching, network opacity, and structural embeddedness on the odds of tie persistence. Model 1 shows that closed-form matches independently predict persistence. Model 2 introduces open-ended matches, demonstrating the value of incorporating both open and closed-form items. Model 3 incorporates network opacity (the number of "Don't know" responses regarding an alter's preferences). Finally, Model 4 introduces structural embeddedness (the number of common neighbors/alter-alter ties shared within ego's network). Model 4 reveals that while structural embeddedness substantially increases the odds of tie survival, cultural matching remains an independent, robust predictor.
```{r}
#| label: tbl-models
#| tbl-cap: "Odds Ratios for Tie Persistence (Main Effects Models)"
modelsummary(
  list("Model 1: Closed" = mod_closed, "Model 2: + Open" = mod_open, "Model 3: + Opacity" = mod_opac_base, "Model 4: + Embeddedness" = mod_embed),
  stars = TRUE,
  exponentiate = TRUE,
...
#| include: false
ctrl_vars_int <- "same_dorm + is_friend + race_homophily + freq_daily + female_factor * alterfemale_factor + duration_c + duration_sq_c + period"
...
datasummary(
  ...
  (`Structural Embeddedness (Common Alters)` = common_alters) +
